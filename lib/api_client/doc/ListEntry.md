...
**transformable** | **bool** | Whether this model has a second printed card it swaps between mid-game — Violent Transformation (Yune Lobravym ⇄ The Beast Within). The client offers the transform button only when true. False for every other model and for Equipment.  | 
**transformed** | **bool** | Which of its two cards this model is currently on the table as: false for the one it was hired as, true for its alternate form. Only ever true during a game — a hired gang always holds the model in its printed form. While true, `identifier`, `name`, `profile_name`, `keywords`, `card_front`, `card_back` and `mage` all describe the alternate form, but `cost` stays that of the hire, and the model keeps its one EntryState, so Life/Will/ Command Points carry across the change untouched. Toggle via PATCH /games/{id}/entries/{list_entry_id}/transform.  | 
**alternateIdentifier** | **String** | The card identifier of this model's other form, or null when it has none. Lets the gang builder preview the other card without a round trip — in the builder the button only flips the card being shown, because the rule transforms a model in play, not at hiring.  | [optional] 
**alternateName** | **String** | The name printed on the other form's card, or null when there is no other form. | [optional] 
**alternateCardFront** | **String** | Front face filename of the other form's card, resolved like `card_front`. Null when there is no other form. | [optional] 
**alternateCardBack** | **String** | Back face filename of the other form's card, resolved like `card_back`. Null when there is no other form. | [optional] 
